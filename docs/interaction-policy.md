...
`preserve` means pointer actions do not request foreground activation. `keyboardPolicy: activate` (the Bundle default) requests foreground activation before `type-text` keyboard fallback and `press-key`; `keyboardPolicy: preserve` keeps typing routed to the selected pid without activation and is reliable only for applications that accept background keyboard events. `targeted` means mouse, drag, and wheel events may be sent only to the exact observed process and window. `visible` enables the separate Agent cursor only when that target application is frontmost. Cursor speed, acceleration, post-arrival click delay, and auto-hide timing are host-owned. The deprecated 0.2.x `cursorMotionMs` field is accepted during upgrade but ignored. The model cannot change any interaction policy through Tool arguments.
...
Visual feedback uses a 28x28 `NSPanel` owned by a dedicated, persistent cursor process. The panel is borderless, nonactivating, click-through, and excluded from normal window cycling; it is not made visible on every Space or over a background target application. It draws the embedded transparent whole-image cursor (`assets/cursor.png`, Cursor arrow plus DeepSeek whale) with its top-left corner at the target point. The first visible move starts at the system cursor's current location without moving that cursor. Later moves start at the prior Agent-cursor point. Distance, requested maximum speed, and acceleration determine a travel time bounded to 48 through 2000 ms; the safety bound takes priority over exact physical units at extreme combinations. Symmetric acceleration/deceleration and a small deterministic Bezier bend produce a legible path. The native move command replies only after arrival, then the provider applies `cursorClickDelayMs` before press feedback. Click input follows that sequence. Drag reaches and presses at the start point first. The native helper then validates the target and waits at a start barrier until the endpoint cursor command has been written; Agent-cursor travel and targeted native drag follow the same bounded Quartz path. Once native mouse-down is possible, caller cancellation waits for the bounded mouse-up instead of terminating the helper mid-gesture. Release validation follows both. The overlay never posts input and never changes the system cursor position.
...
- The Agent cursor is scoped to the current frontmost application, one active Space, and the exact observed window. `cursorAutoHideMs: 0` keeps it visible only while all three remain valid; a foreground change hides it within the periodic target check.
