...
export function collected(reader) {
    if (reader === undefined)
        return '';
 ...
 */
export async function prepareNativeHelper(client, signal) {
...
export async function buildManagedHelper(client, signal) {
    const script = fileURLToPath(new URL('../../scripts/build-native.mjs', import.meta.url));
    const handle = client.ctx.subprocess.spawn({
...
//# sourceMappingURL=binding.native-manifest.js.map