...
import { allocateScreenshotPath, describeScreenshot } from "../optics/optics.artifact.js";
import { diffElements } from "../optics/optics.drift.js";
import { publicElements } from "../optics/optics.snapshot.js";
/** Observe (or reuse), project, store, and return one observation. */
export async function captureObservation(host, app, request, context, sourceTool, preObserved) {
    ...
    const artifact = backend.screenshot === undefined
        ? undefined
        : await describeScreenshot(backend.screenshot.path, backend.screenshot.width, backend.screenshot.height, host.config.maxScreenshotBytes, sourceTool);
    const observation = {
        observationId,
        app: backend.app,
...
//# sourceMappingURL=conductor.capture.js.map