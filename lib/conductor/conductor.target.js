...
import { allowsTargetRebind, requiresElement, requiresForegroundPermission, requiresPointerInput, targetHandle, targetIndex, } from "../motion/motion.envelope.js";
/** Whether a descriptor locator still names exactly this provider element. */
function locatorNamesElement(locator, element) {
    if (element === undefined)
        return false;
    if (locator.length !== element.locator.length)
        return false;
...
function elementAtIndex(observation, index) {
    if (index === undefined)
        return undefined;
...
function elementForDescriptor(observation, descriptor) {
 ...
 */
export function selectActionTarget(stored, action, config) {
...
//# sourceMappingURL=conductor.target.js.map