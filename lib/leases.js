...
export class ComputerLeaseManager {
    ...
    async ensureInteractive(agent, app, scope, toolName, callId, signal) {
        ...
        if (approvalPolicy(this.ctx, agent) === 'never') {
            throw new ComputerUseError('COMPUTER_PERMISSION_REQUIRED', `${scope} access for ${app.name} is blocked because approval prompts are disabled in this Session (approval/policy: never, e.g. the danger-full-access preset); add "${app.bundleId}" to the computer-use grants in Settings, or switch the permission preset to one with approval ask`);
        }
    ...
    async persist(agent, app, decision) {
        ...
        try {
            ...
            if (error instanceof ComputerUseError)
                throw error;
            throw new ComputerUseError('COMPUTER_PERMISSION_REQUIRED', `${purpose} for ${app.name} could not be persisted after the approval audit: ${boundedFailure(error)}; configure working Session persistence and @deepseek-ai/dsh-storage-domain before retrying`, { cause: error });
        }
    }
    storageRequired(app, scope, purpose) {
        return new ComputerUseError('COMPUTER_PERMISSION_REQUIRED', `${scope} access for ${app.name} requires ctx.storageDomain to persist ${purpose}; compose @deepseek-ai/dsh-storage-domain or add an exact static grant for "${app.bundleId}" before retrying`);
    }
    enqueueDecision(sessionId, operation) {
        ...
        return result.finally(() => {
            if (this.decisionTails.get(sessionId) === tail)
                this.decisionTails.delete(sessionId);
        });
    }
