...
import { MAX_SET_VALUE_CHARACTERS, waitConditionDefined } from "./motion.budget.js";
function isElementAddressed(action) {
    switch (action.kind) {
        case 'click':
        case 'scroll':
        case 'set-value':
...
export function targetIndex(action) {
    if (!isElementAddressed(action))
        return undefined;
    return action.elementIndex;
}
export function targetHandle(action) {
    if (!isElementAddressed(action))
        return undefined;
    return action.targetHandle;
}
export function allowsTargetRebind(action) {
    return isElementAddressed(action) && action.allowRebind === true;
}
export function requiresElement(action) {
    return action.kind === 'set-value' || action.kind === 'perform-action';
}
export function requiresPointerInput(action, element) {
    switch (action.kind) {
        case 'scroll':
...
export function requiresForegroundPermission(action) {
...
//# sourceMappingURL=motion.envelope.js.map