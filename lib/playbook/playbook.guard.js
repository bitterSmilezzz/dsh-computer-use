...
import { COMPUTER_USE_SKILL_CONTENT, COMPUTER_USE_SKILL_NAME } from "./playbook.skill.js";
...
function matchesShellOcrTooling(segment) {
...
function adHocOcrCommand(value) {
    ...
    if (segments.some(matchesShellOcrTooling))
        return true;
...
function namesBundledSkill(value) {
...
function namesBundledSkillInJson(raw) {
    try {
...
function carriesSkillBody(blocks) {
 ...
 */
function eventProvesActivation(event, skillCalls) {
    switch (event.type) {
        case 'user/message': {
...
function loadedFromEvents(events) {
    ...
    proven(session) {
        ...
        if (memo !== undefined) {
            ...
            if (memo.snapshot === events
                ...
                && memo.last === events[events.length - 1])
                return false;
        }
...
export class ComputerUseExposure {
    activationTool;
    scopes = new Map();
    verdicts = new ActivationVerdicts();
    ctx;
    createTools;
    installed = false;
    ...
    shellOcrRefusal(exec) {
        const { agent } = exec;
        if (exec.name !== 'bash' || agent === undefined)
            return undefined;
        if (!this.verdicts.proven(agent.session) || !adHocOcrCommand(exec.arguments))
            return undefined;
    ...
    activate(agent) {
        ...
        if (scope.active)
            return { activated: false, tools: [...scope.toolNames] };
    ...
    releaseScope(scope) {
        ...
        for (const dispose of scope.toolDisposers.reverse())
            dispose();
    }
}
//# sourceMappingURL=playbook.guard.js.map