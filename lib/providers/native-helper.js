...
import { ComputerUseError, computerUseError, computerUseErrorCode } from "../errors.js";
/**
...
export class NativeHelperClient {
    ...
    async invoke(request, signal) {
        ...
        if (envelope.ok !== true)
            throw helperFailure(envelope.error);
    ...
    async prepareDrag(request, signal, readinessTimeoutMs) {
        ...
        const readLine = async (readSignal) => await new Promise((resolveLine, rejectLine) => {
            const onAbort = () => { rejectLine(readSignal.reason); };
            ...
            void iterator.next().then(next => {
                ...
                if (next.done === true)
                    rejectLine(new Error('native drag helper closed before replying'));
                else if (Buffer.byteLength(next.value) > 4 * 1024 * 1024)
                    rejectLine(new Error('native drag helper response exceeded its protocol limit'));
                else
                    resolveLine(next.value);
            }, error => {
                readSignal.removeEventListener('abort', onAbort);
                rejectLine(error);
            });
        });
        const writeLine = async (line) => await new Promise((resolveWrite, rejectWrite) => {
            stdin.write(`${line}\n`, error => {
        ...
        try {
            ...
            const ready = JSON.parse(await readLine(readySignal));
    ...
    async executeCursorCommand(command, signal, onWritten) {
        const prepared = this.prepared ?? await this.prepare(signal);
        if (this.disposed)
            throw new ComputerUseError('COMPUTER_PROVIDER_FAILURE', 'native cursor overlay client is disposed');
