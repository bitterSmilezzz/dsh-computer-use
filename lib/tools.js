...
import { ComputerConfirmationToken, ComputerObservationId, ComputerTargetHandle, } from "./types.js";
...
function deferVisionHandoff(exec, artifact) {
...
function actionOutput() {
    return {
        ...
        render: renderJson,
        presentationMeta: (_args, value) => value.observation.screenshot === undefined
            ? {}
...
export function createComputerUseTools(service) {
    ...
    const click = defineTool({
        ...
        output: actionOutput(),
        execute: (args, exec) => service.act({ kind: 'click', ...actionBase(args), ...elementTarget(args), ...args.x === undefined ? {} : { x: args.x }, ...args.y === undefined ? {} : { y: args.y }, ...args.coordinateSpace === undefined ? {} : { coordinateSpace: args.coordinateSpace }, ...args.button === undefined ? {} : { button: args.button }, ...args.clickCount === undefined ? {} : { clickCount: args.clickCount }, ...args.modifiers === undefined ? {} : { modifiers: args.modifiers }, ...args.allowCoordinateFallback === undefined ? {} : { allowCoordinateFallback: args.allowCoordinateFallback } }, contextOf(exec)),
    ...
    const drag = defineTool({
        name: 'computer_drag',
        description: 'Drag by routing mouse events only to the selected app process between two points in the observed-window or screen-global coordinate space. The system cursor is not moved. Pass modifiers to hold command, control, option, or shift for the whole gesture; a modified drag stays a real pointer drag.',
        parameters: {
            observationId: { type: 'string', required: true },
            fromX: { type: 'number', required: true },
            fromY: { type: 'number', required: true },
        ...
        output: actionOutput(),
        execute: (args, exec) => service.act({ kind: 'drag', ...actionBase(args), fromX: args.fromX, fromY: args.fromY, toX: args.toX, toY: args.toY, ...args.coordinateSpace === undefined ? {} : { coordinateSpace: args.coordinateSpace }, ...args.modifiers === undefined ? {} : { modifiers: args.modifiers } }, contextOf(exec)),
    ...
    const wait = defineTool({
        name: 'computer_wait',
        description: 'Wait until one bounded Accessibility condition on the referenced app holds, then return fresh state without mutating the app. Prefer this over repeated computer_observe polling. Set absent=true to wait for a loading indicator, banner, or dialog to disappear instead of for it to appear. A condition must set at least one of text, elementRole, elementTitle, or elementValue; an empty condition fails immediately without waiting. timeoutMs defaults to the host maxSettleMs and may be raised up to the host maxWaitMs; on timeout the error asks you to observe the current state rather than guess.',
        parameters: {
            observationId: { type: 'string', required: true },
            condition: {
                type: 'object',
