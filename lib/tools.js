...
import { ComputerConfirmationToken, ComputerObservationId, } from "./types.js";
...
export function createComputerUseTools(service) {
    ...
    const click = defineTool({
        name: 'computer_click',
        description: 'Click an element from the exact observation, preferring AXPress, or an observed-window coordinate. The successful result already contains the fresh post-click observation.',
        parameters: {
            observationId: { type: 'string', required: true },
            elementIndex: { type: 'integer' },
            x: { type: 'number' },
    ...
    const typeText = defineTool({
        ...
        description: 'Type Unicode into the currently focused control without reading or replacing the clipboard. Focus a control using fresh state first. The result does not echo the supplied text.',
        parameters: {
            observationId: { type: 'string', required: true },
            text: { type: 'string', required: true },
            ...sensitiveParameters,
    ...
    const pressKey = defineTool({
        ...
        description: 'Press one validated key or chord in the selected app. Use documented key names and read the returned fresh observation.',
        parameters: {
            observationId: { type: 'string', required: true },
            key: { type: 'string', enum: keyNames, required: true },
            modifiers: { type: 'array', items: { type: 'string', enum: ['command', 'control', 'option', 'shift'] } },
    ...
    const scroll = defineTool({
        ...
        description: 'Scroll at an observed element or observed-window coordinate in a bounded direction and page count.',
        parameters: {
            observationId: { type: 'string', required: true },
            elementIndex: { type: 'integer' },
            x: { type: 'number' },
    ...
    const drag = defineTool({
        name: 'computer_drag',
        description: 'Drag between two points in the exact observed-window coordinate space.',
        parameters: {
            observationId: { type: 'string', required: true },
            fromX: { type: 'number', required: true },
            fromY: { type: 'number', required: true },
