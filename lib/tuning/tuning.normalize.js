 ...
 */
function resolveSettlement(config) {
    ...
    if (settleMs > maxSettleMs)
        refuse('settleMs must be no greater than maxSettleMs');
    const maxWaitMs = Math.max(boundedInteger('maxWaitMs', config.maxWaitMs ?? 30_000, 100, 600_000), maxSettleMs);
    return { settleMs, maxSettleMs, maxWaitMs };
}
/** How much of one provider tree is read before the observation counts as truncated. */
function resolveObservationLimits(config) {
    return {
...
function resolveWorkspace(config) {
    ...
    if (helperPath !== undefined && helperPath.length === 0)
        refuse('helper.path must not be empty');
    return {
        artifactRoot,
        helper: {
 ...
 */
function resolveInteraction(config) {
    const source = config.interaction;
    const focusPolicy = boundedOption('interaction.focusPolicy', source?.focusPolicy ?? 'preserve', ['preserve', 'activate']);
    const keyboardPolicy = boundedOption('interaction.keyboardPolicy', source?.keyboardPolicy ?? 'preserve', ['preserve', 'activate']);
    const pointerInputPolicy = boundedOption('interaction.pointerInputPolicy', source?.pointerInputPolicy ?? 'targeted', ['deny', 'targeted']);
    const cursorVisualization = boundedOption('interaction.cursorVisualization', source?.cursorVisualization ?? 'visible', ['hidden', 'visible']);
    const legacyMotion = source?.cursorMotionMs;
    if (legacyMotion !== undefined)
        boundedInteger('interaction.cursorMotionMs', legacyMotion, 0, 2_000);
    return {
        focusPolicy,
        keyboardPolicy,
        pointerInputPolicy,
 ...
 */
function resolveAppAccess(config) {
    const named = new Set();
    const grants = [];
    for (const requested of config.grants ?? []) {
        ...
        if (named.has(bundleId))
            refuse(`duplicate app grant for ${bundleId}`);
        named.add(bundleId);
...
//# sourceMappingURL=tuning.normalize.js.map