...
import { NativeHelperClient, type PreparedNativeDrag } from './binding.native-transport.ts'

/** Health facts as the helper reports them; the manifest supplies the fallback version. */
type NativeHealth = Pick<BackendHealth, 'accessibility' | 'screenRecording'> & {
  helperVersion: string
}

/** Observation plus how the provider reached the foreground, as one helper reply. */
type NativeCursorActivation = BackendCursorActivation
 ...
 */
class CursorOutcome {
  private worst: CursorVisibility = { visible: true }

  /** Keep the first refusal; a later reply cannot improve on it. */
  note(response: CursorVisibility): void {
    if (!response.visible && this.worst.visible) this.worst = response
  }

  get value(): CursorVisibility {
    return this.worst
  }
}
...
export class MacOSBackend implements ComputerUseBackend {
   ...
   */
  private async callHelper<T>(command: string, payload: Record<string, unknown>, signal: AbortSignal): Promise<T> {
    return await this.client.invoke<T>({ command, ...payload }, signal)
  }
  ...
  async health(signal: AbortSignal): Promise<BackendHealth> {
    ...
    const reported = await this.callHelper<NativeHealth>('health', {}, signal)
    return {
  ...
  async openSettings(kind: 'accessibility' | 'screen-recording', signal: AbortSignal): Promise<void> {
    await this.callHelper<null>('open-settings', { kind }, signal)
  }
  ...
  async resolveApp(selector: ComputerAppSelector, signal: AbortSignal): Promise<ComputerAppIdentity> {
    return await this.callHelper<ComputerAppIdentity>('resolve-app', { selector }, signal)
  }
  ...
  async listApps(signal: AbortSignal): Promise<ComputerAppSummary[]> {
    return await this.callHelper<ComputerAppSummary[]>('list-apps', {}, signal)
  }
  ...
  async observe(app: ComputerAppIdentity, options: BackendObserveOptions, signal: AbortSignal): Promise<BackendObservation> {
    return await this.callHelper<BackendObservation>('observe', { app, options }, signal)
  }
  ...
  async activateForCursor(
    ...
    return await this.callHelper<NativeCursorActivation>('activate-for-cursor', {
      app,
      expectedStateHash,
      options,
  ...
  private get actionLimits(): { maxNodes: number; maxDepth: number; maxTextBytes: number } {
    return {
   ...
   */
  private actEnvelope(request: BackendActionRequest): { command: 'act'; request: Record<string, unknown> } {
    return {
      command: 'act',
      request: {
        ...request,
  ...
  async act(request: BackendActionRequest, signal: AbortSignal): Promise<BackendActionResult> {
    if (request.action.kind === 'drag') return await this.runNativeDrag(request, signal)
    return await this.client.invoke<BackendActionResult>(this.actEnvelope(request), signal)
  }

  /** A drag runs through the explicit start barrier, never the one-shot `act` invocation. */
  private async runNativeDrag(request: BackendActionRequest, signal: AbortSignal): Promise<BackendActionResult> {
  ...
  private async prepareNativeDrag(request: BackendActionRequest, signal: AbortSignal): Promise<PreparedNativeDrag<BackendActionResult>> {
    return await this.client.prepareDrag<BackendActionResult>(
      this.actEnvelope(request),
      signal,
      this.config.actionTimeoutMs + 2_000,
    )
  }
   ...
   */
  private async trackDragCursor(
    cursor: BackendCursorAction & { kind: 'drag' },
    execution: PreparedNativeDrag<BackendActionResult>,
    signal: AbortSignal,
  ...
  async visualizeCursor(action: BackendCursorAction, phase: CursorPhase, signal: AbortSignal): Promise<CursorVisibility> {
  ...
  private moveCommand(action: BackendCursorAction, point: { x: number; y: number }): Record<string, unknown> {
    return {
      op: 'move',
      x: point.x,
      y: point.y,
  ...
  private settleCommand(action: BackendCursorAction): Record<string, unknown> {
    return {
  ...
  private pressCommand(action: BackendCursorAction): Record<string, unknown> {
    return {
      op: 'press',
      autoHideMs: 0,
      targetPid: action.targetPid,
  ...
  private async visualizeCursorPhase(
    ...
    switch (phase) {
      ...
      default: {
        ...
        if (from === undefined) return { visible: false, reason: 'this action has no cursor position to show' }
        ...
        if (!outcome.value.visible || action.kind === 'scroll') return outcome.value
