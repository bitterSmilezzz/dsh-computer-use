...
import type { BackendActionResult, BackendObservation, ComputerUseBackend } from '../binding/binding.port.ts'
import { ComputerConfirmationManager } from '../custody/custody.consent.ts'
import { Ledger, type StoredObservation } from '../custody/custody.ledger.ts'
import { ComputerUseError, computerUseError } from '../charter/charter.fault.ts'
import { ComputerLeaseManager } from '../custody/custody.lease.ts'
import type { ResolvedComputerUseConfig } from '../tuning/tuning.schema.ts'
import { allocateScreenshotPath } from '../optics/optics.artifact.ts'
import { resolveComputerTarget } from '../optics/optics.locate.ts'
import { captureObservation, type CaptureHost } from './conductor.capture.ts'
import { CursorFlight } from './conductor.cursor.ts'
import { ConductorQueue } from './conductor.queue.ts'
import { selectActionTarget } from './conductor.target.ts'
import {
  applyHealth,
  degradedHealth,
  describeStatus,
  unavailableHealth,
  type HealthSnapshot,
} from './conductor.health.ts'
import {
  allowsTargetRebind,
  requiresPointerInput,
  validateAction,
} from '../motion/motion.envelope.ts'
import { matchesWait, resolveWaitTimeout } from '../motion/motion.budget.ts'
import { runSettleLoop, settleEffect } from '../motion/motion.settle.ts'
import {
 ...
 */
function waitTimeoutDetail(
  condition: Extract<ComputerActionRequest, { kind: 'wait' }>['condition'],
  budget: number,
): string {
...
export class ComputerUseService extends Service {
  ...
  private async actNow(
    ...
    let resolution: ComputerTargetResolutionResult | undefined = target.resolution
    ...
    const settle = await runSettleLoop({
      ...
      signal,
      allocateScreenshotPath: () => allocateScreenshotPath(
        context.workspace,
  ...
  private rejectReboundSensitiveTarget(
    ...
    if (action.sensitive !== true || resolution?.targetChanged !== true) return
