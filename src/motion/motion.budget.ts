...
export function waitConditionDefined(condition: ComputerWaitCondition): boolean {
...
function waitConditionMatches(observation: BackendObservation, condition: ComputerWaitCondition): boolean {
  ...
  if (condition.elementValue !== undefined && !observation.elements.some(element => element.value === condition.elementValue)) return false
  return true
}

export function matchesWait(observation: BackendObservation, action: Extract<ComputerActionRequest, { kind: 'wait' }>): boolean {
...
export function resolveWaitTimeout(
  ...
  ceiling: { maxSettleMs: number; maxWaitMs: number },
): number {
  ...
  if (!Number.isInteger(resolved) || resolved < 100 || resolved > ceiling.maxWaitMs) {
    throw new ComputerUseError(
      ...
      `wait timeoutMs must be an integer between 100 and ${ceiling.maxWaitMs} milliseconds`,
    )
  }
  return resolved
}
