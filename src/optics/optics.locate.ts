...
import type { ComputerRect, ComputerTargetResolutionResult } from '../charter/charter.index.ts'
...
/** One element of a fresh observation paired with the descriptor it projects to. */
interface Candidate {
  element: BackendElement
  descriptor: ComputerTargetDescriptor
}

/** One entry of a descriptor's ancestor chain, as stored. */
type AncestorFingerprint = ComputerTargetDescriptor['ancestorFingerprint'][number]

/** Refuse one resolution with a published target error. */
function refused(code: 'COMPUTER_TARGET_AMBIGUOUS' | 'COMPUTER_TARGET_LOW_CONFIDENCE', message: string): never {
...
function ambiguous(tier: string, candidateCount: number): never {
  return refused(
    'COMPUTER_TARGET_AMBIGUOUS',
    `${tier} resolution found ${candidateCount} candidates in the selected process and window`,
  )
}

/** No candidate could be vouched for; the message says which evidence was missing. */
function tooWeak(candidateCount: number, reason: string): never {
  return refused(
...
function hit(
  ...
  mode: ComputerTargetResolutionResult['mode'],
  confidence: number,
 ...
 */
function requireSameSubject(original: BackendObservation, fresh: BackendObservation): void {
  ...
  if (sameWindow(original.window, fresh.window)) return
 ...
 */
function ancestorChain(element: BackendElement, byLocator: Map<string, BackendElement>): AncestorFingerprint[] {
  const chain: AncestorFingerprint[] = []
 ...
 */
function windowRelativeFrame(element: BackendElement, observation: BackendObservation): ComputerRect | undefined {
  const frame = element.frame
  if (frame === undefined) return undefined
  const window = observation.window
  if (window === undefined) return { ...frame }
  return {
...
/**
 * One rebinding tier. Returns the resolution it can vouch for, `undefined` when
 * the tier does not apply to this descriptor or found nothing, and refuses
 * outright when the evidence is ambiguous or too weak to act on.
 */
type RebindTier = (
  expected: ComputerTargetDescriptor,
  candidates: readonly Candidate[],
  fresh: BackendObservation,
) => ResolvedComputerTarget | undefined
 ...
 */
const byNativeIdentifier: RebindTier = (expected, candidates, fresh) => {
  if (expected.nativeIdentifier === undefined) return undefined
  const matches = candidates.filter(candidate => candidate.descriptor.nativeIdentifier === expected.nativeIdentifier)
  if (matches.length > 1) ambiguous('native identifier', matches.length)
 ...
 */
const bySemanticIdentity: RebindTier = (expected, candidates, fresh) => {
  if (expected.accessibleName === undefined) return undefined
  const matches = candidates.filter(candidate => sameSemanticIdentity(expected, candidate.descriptor))
  if (matches.length > 1) ambiguous('semantic', matches.length)
  if (matches.length === 0) {
...
const REBIND_TIERS: readonly RebindTier[] = [byNativeIdentifier, bySemanticIdentity]
...
export function resolveComputerTarget(
  ...
  const candidates: Candidate[] = fresh.elements.map(element => ({
    element,
