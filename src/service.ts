...
function waitConditionDefined(condition: ComputerWaitCondition): boolean {
...
export class ComputerUseService extends Service {
   ...
   */
  private validateAction(action: ComputerActionRequest): void {
  ...
  private async wait(
    ...
    if (!Number.isInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > this.config.maxWaitMs) {
      throw new ComputerUseError(
        'COMPUTER_INVALID_ARGUMENT',
        `wait timeoutMs must be an integer between 100 and ${this.config.maxWaitMs} milliseconds`,
      )
    }
    const started = Date.now()
