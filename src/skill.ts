...
7. Prefer computer_wait over polling computer_observe. Its condition matches on
   text, elementRole, elementTitle, or elementValue, and absent=true waits for
   that match to disappear, which is how a loading indicator or dialog closing is
   observed. A condition with no matcher fails immediately, and timeoutMs may be
   raised above the default up to the host maxWaitMs. computer_click and
   computer_drag accept the same command/control/option/shift modifiers as
   computer_press_key, and a modified click or drag is always real
   target-process pointer input.
