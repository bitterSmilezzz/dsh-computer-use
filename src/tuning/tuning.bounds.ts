...
export type BoundedNumericField =
  | 'observationTtlMs'
  | 'confirmationTtlMs'
  | 'actionTimeoutMs'
  | 'settleMs'
...
export const NUMERIC_BOUNDS: Record<BoundedNumericField, NumericBound> = {
...
export function boundsOf(field: BoundedNumericField): NumericBound {
...
export function withinBounds(field: BoundedNumericField, value: number): boolean {
  const bound = NUMERIC_BOUNDS[field]
  if (bound.allowZero && value === 0) return true
